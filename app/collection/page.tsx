...
const ALBUMS: Album[] = [
  ...
  { id:"revakplim", title:"Rev ak Plim", year:"2018", cover:"/revakplim-cover.jpg", zipUrl:`${BASE}/revakplim/Rev-Ak-Plim.zip`,
    tracks:[
      ...
      {title:"Pa gen Rev ki Two Gran ft Da.G",url:`${BASE}/revakplim/12-Pa%20gen%20Rev%20ki%20Two%20Gran%20ft%20Da.G.mp3`},
  ...
  { id:"eskal1", title:"Eskal I", year:"2022", cover:"/eskal1-cover.jpg", zipUrl:`${BASE}/eskal1/Eskal-I.zip`,
    tracks:[
      ...
      {title:"Chimen Pedi Tan",url:`${BASE}/eskal1/03%20-%20Chimen%20P%C3%A8di%20Tan.mp3`},
      ...
      {title:"Denye Pasaje",url:`${BASE}/eskal1/10%20-%20Deny%C3%A8%20Pasaje.mp3`},
...
function Gate({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center"
      ...
      <p className="font-mono text-[#B8430E] text-[10px] uppercase tracking-[0.3em] mb-2">Koleksyon Konplet</p>
      ...
      <p className="text-[rgba(232,223,200,0.55)] text-sm max-w-xs mb-8 leading-relaxed">Antre imel ou te itilize pou achte koleksyon konplet la.</p>
      <form onSubmit={submit} className="w-full max-w-sm flex flex-col gap-3">
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? "N ap verifye..." : "Jwenn Akse"}
        </button>
...
function AlbumSection({ album, currentUrl, onPlay }: { album:Album; currentUrl:string; onPlay:(url:string,albumId:string,idx:number)=>void; }) {
  const isActive = album.tracks.some(t => t.url === currentUrl);
  const [open, setOpen] = useState(album.id === "manifesto");

  return (
    <div className="mb-4 bg-[#1C1410] border border-[rgba(212,180,131,0.1)] rounded-sm overflow-hidden">
      <button onClick={() => setOpen(!open)}
        ...
        <div className="relative w-14 h-14 flex-shrink-0 rounded-sm overflow-hidden border border-[rgba(212,180,131,0.15)]">
          {album.cover ? <Image src={album.cover} alt={album.title} fill className="object-cover"/> : <div className="w-full h-full bg-[#0A0705]"/>}
        </div>
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <button onClick={e=>{e.stopPropagation(); onPlay(album.tracks[0].url, album.id, 0);}}
            className="w-9 h-9 rounded-full bg-[#6B0F12] hover:bg-[#B8430E] transition-colors flex items-center justify-center text-[#E8DFC8] text-sm">▶</button>
          <a href={album.zipUrl} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()}
            className="font-mono text-[rgba(212,180,131,0.4)] hover:text-[#D4B483] text-[10px] uppercase tracking-wider transition-colors">ZIP</a>
          ...
          {album.tracks.map((t,i) => {
            ...
            return (
              <button key={i} onClick={() => onPlay(t.url, album.id, i)}
                className={`w-full flex items-center gap-3 px-4 py-2.5 transition-colors text-left group ${isPlaying ? "bg-[rgba(107,15,18,0.3)]" : "hover:bg-[rgba(212,180,131,0.04)]"}`}>
                <span className={`font-mono text-[10px] w-6 flex-shrink-0 text-center ${isPlaying ? "text-[#B8430E]" : "text-[rgba(212,180,131,0.3)]"}`}>
                  {isPlaying ? "▶" : String(i+1).padStart(2,"0")}
                </span>
                <span className={`flex-1 text-xs ${isPlaying ? "text-[#E8DFC8] font-display" : "text-[rgba(232,223,200,0.6)] group-hover:text-[#E8DFC8]"}`}>{t.title}</span>
              </button>
            );
          })}
        </div>
  ...
  return (
    <div className="min-h-screen pb-28" style={{background:"radial-gradient(ellipse at 50% 0%, rgba(107,15,18,0.25) 0%, rgba(10,7,5,0) 45%)"}}>
      <div className="max-w-lg mx-auto px-4 pt-12">
        <p className="font-mono text-[#B8430E] text-[10px] uppercase tracking-[0.3em] mb-1 text-center">Koleksyon Konplet</p>
        ...
        {ALBUMS.map(a => (
          <AlbumSection key={a.id} album={a} currentUrl={currentUrl} onPlay={handlePlay}/>
        ))}
      </div>
      {currentUrl && (
          ...
          <div className="flex items-center gap-3 px-4 py-3">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <div className="relative w-11 h-11 flex-shrink-0 rounded-sm overflow-hidden border border-[rgba(212,180,131,0.2)]">
                {currentAlbum?.cover ? <Image src={currentAlbum.cover} alt={currentAlbum.title} fill className="object-cover"/> : <div className="w-full h-full bg-[#1C1410]"/>}
              </div>
              <div className="min-w-0">
                <p className="font-display text-[#E8DFC8] text-sm truncate">{currentTrack?.title}</p>
                <p className="font-mono text-[rgba(212,180,131,0.45)] text-[10px] truncate">{currentAlbum?.title}</p>
              </div>
            </div>
            <div className="flex items-center gap-4 flex-shrink-0">
              ...
              <button onClick={togglePlay} className="w-10 h-10 rounded-full bg-[#E8DFC8] hover:bg-white transition-colors flex items-center justify-center text-[#0A0705] text-base font-bold">{playing?"⏸":"▶"}</button>
...
export default function CollectionPage() {
  ...
  return unlocked ? <CollectionPlayer /> : <Gate onSuccess={() => setUnlocked(true)} />;
}
